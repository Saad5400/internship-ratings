    ...
    <div class="flex items-center gap-1" x-data="{
        ...
        @for($i = 1; $i <= 5; $i++)
            <button
                ...
                aria-label="{{ $i }} من 5">
                <svg
                    :class="((hover || value) >= {{ $i }}) ? 'text-amber-400' : 'text-slate-200 dark:text-slate-700'"
        ...
        @endfor
        <span class="ms-3 text-sm font-semibold text-slate-700 tabular-nums dark:text-slate-300" x-show="value" x-cloak>
