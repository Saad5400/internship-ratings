...
<article class="flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-xs">
    ...
    <div class="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4">
        <x-admin.moderation-actions
            :record="$company"
            type="company"
            :confirm-reject="$linkedRatings > 0 ? 'لدى هذه الجهة '.trans_choice('counts.ratings', $linkedRatings).'؛ برفضها ستُخفى هي وتقييماتها من الموقع. يمكنك أولاً نقل التقييمات إلى جهة أخرى من بطاقة كل تقييم.' : null"
        />
        <span class="inline-flex items-center gap-1 text-xs {{ $linkedRatings > 0 ? 'font-medium text-amber-600' : 'text-slate-400' }}" title="عدد التقييمات المرتبطة بهذه الجهة">
            ...
            {{ trans_choice('counts.ratings', $linkedRatings) }}
        </span>
    </div>
</article>
