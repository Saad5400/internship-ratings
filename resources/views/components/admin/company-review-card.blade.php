...
<article class="flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-xs">
    ...
    <div class="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4">
        <x-admin.moderation-actions :record="$company" type="company" />
        <span class="inline-flex items-center gap-1 text-xs text-slate-400" title="عدد التقييمات المرتبطة">
            ...
            {{ $company->ratings_count ?? $company->ratings()->count() }} تقييم
        </span>
    </div>
</article>
